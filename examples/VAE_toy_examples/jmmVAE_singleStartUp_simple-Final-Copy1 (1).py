...
from tools import nb_tools, viz, loader, build_model, sanity, sampling, custom_variational_layer
...
# MMVAE
#working
beta_shared = .001 # 0.1 # KL of multi modal encoder
beta_uni = .001 # 0.001 # KL of uni modal encoder
alpha = .001 # 1. # KL between uni- and shared encoder
...
beta_shared = np.float(os.environ['BETA_SHARED'])
beta_uni = np.float(os.environ['BETA_UNI'])
alpha = np.float(os.environ['ALPHA'])
gamma = np.float(os.environ['GAMMA'])
...
x1_plot = ax[0].scatter(x_train_shared[:,0], x_train_shared[:,1], c = gt_train, cmap = 'tab10')
x2_plot = ax[1].scatter(w_train_shared[:,0], w_train_shared[:,1], c = gt_train, cmap = 'tab10')
...
shared_test_encoded_var = np.sum(kl_loss_n(encoded_1, _shared_test_encoded_log_var), axis=1)
x_test_encoded_var      = np.sum(kl_loss_n(encoded_2, _x_test_encoded_log_var), axis=1)
w_test_encoded_var      = np.sum(kl_loss_n(encoded_3, _w_test_encoded_log_var), axis=1)
...
for axes in ax:
    axes.set_xticks([])
    axes.set_yticks([])
plt.savefig(prefix_str + "variances" + setup_str + ".svg")
plt.show(block=False)
