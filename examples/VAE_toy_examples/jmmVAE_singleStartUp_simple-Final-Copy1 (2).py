...
try:
    ...
    _epochs = np.float(os.environ['EPOCHS'])
...
for idx in np.arange(epochs):
    ...
    vae.fit(x = [x_train_shared, w_train_shared, x_train_shared, w_train_shared],
        ...
        validation_data=([x_test, w_test, x_test, w_test], None))
# ELBO test
elbo_xw, elbo_x, elbo_w = elbo_check(model_obj, x_test, w_test, elbo_xw, elbo_x, elbo_w)
...
# Show the elbo
plt.plot(np.array([elbo_xw, elbo_x, elbo_w]).T)
plt.legend(('elbo_xw', 'elbo_x', 'elbo_w'))
